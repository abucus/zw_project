...
_project_num_range = [40, 45]
_resource_type_num_range = [50, 150]
_non_renew_resource_type_num_range = [50, 140]
_project_review_range = [1, 5]
_project_review_range2 = [300, 350]
_activity_num_range = [30, 45]
...
project_n = uniform_number(*_project_num_range)  # project number
project_list = ["P" + str(i) for i in range(1, project_n + 1)]
# print project_list

resource_type_n = uniform_number(*_resource_type_num_range)  # NK type number
resource_type_list = ["NK0g" + str(i) for i in range(1, resource_type_n + 1)]

non_renew_resource_type_n = uniform_number(*_non_renew_resource_type_num_range)  # RK type number
...
for row in project_list:
    writer.writerow([row, uniform_number(*_project_review_range), uniform_number(*_project_review_range2)])  # modify project_revie_due
...
for row in project_list:
    i = 0
    this_activity_num = uniform_number(*_activity_num_range)  # activity number of each project
    ...
    for k in range(this_activity_num):
        ...
        resource_require_list = random.sample(resource_type_list, uniform_number(10,
                                                                                 resource_type_n))  # NK activity required resource number and list
        non_renew_resource_require_list = random.sample(non_renew_resource_type_list, uniform_number(10,
                                                                                                     non_renew_resource_type_n))  # RK activity required resource number ad list
        this_resource_list = ''
...
for row in resource_type_list:
    ...
    for row2 in range(current_supplier, current_supplier + this_resource_supplier_num):
        writer4.writerow([row, "S" + str(row2), int(normal_number(500, 30)),
                          int(normal_number(20, 5))])  # resource- supplier capacity & release time
        arr1 = []
        arr2 = []
        arr1.append(row)
