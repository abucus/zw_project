...
G=[]
for network_num in [3]:#range(5,100): #activity number size from 5 to 20
    for seedss in range(100):
        G1=nx.fast_gnp_random_graph(network_num,0.4,seed=seedss,directed=True)
        ...
        if DAG not in G and nx.is_directed_acyclic_graph(DAG):
            G.append(DAG)
            file=open('generated//'+'size_'+str(network_num)+'_'+str(seedss)+'.csv','w', newline='')
